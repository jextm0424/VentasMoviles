...
public class MaterialController {
	...
	public ModelAndView agregarMaterial(
			@Valid @ModelAttribute("matModel") Material material,
			BindingResult rs) {
	...
	public ModelAndView materialList(
		...
		if (tipoFiltro == null || parametro == null) {
			model.addObject("materiales", materialService.getAll());
			return model;
		} else {
			switch (tipoFiltro) {
			case "E":
			...
			case "I":
				...
				try {
					materiales.add(materialService.findOne(Integer
							.parseInt(parametro)));
